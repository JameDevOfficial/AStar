...
def create_windows_exe(love_file, love_version, arch, output_dir):
    ...
    game_exe = love_dir / "astar.exe"
    
    print(f"Creating astar.exe ({arch}-bit)...")
    ...
    # Create zip archive
    output_zip = output_dir / f"astar-win{arch}.zip"
...
def main():
    ...
    if is_wsl:
        ...
        lovejs_cmd = f"npx love.js.cmd -c \"{love_file_win}\" \"{web_win}\" --title \"AStar\""
    else:
        lovejs_cmd = f"npx love.js.cmd -c \"{love_file}\" \"{web}\" --title \"AStar\""
    ...
    checks = {
        ".love file": love_file,
        "Windows 32-bit": executables / "astar-win32.zip",
        "Windows 64-bit": executables / "astar-win64.zip", 
